...
docker-compose exec -T stats datacube product add https://raw.githubusercontent.com/digitalearthafrica/config/master/products/esa_s2_l2a.yaml
...
docker-compose exec -T stats odc-stats save-tasks --grid albers_africa_10 --year 2019 --tiles 5:8,5:8 --overwrite s2_l2a test-run.db
echo "Checking a job run"
docker-compose exec -T stats odc-stats run-pq --location file:///tmp ./test-run.db 0
